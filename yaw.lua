...
ui_new_label("LUA", "B", "-----------[ZHAG-YAW]-----------")
-----------回溯拓展-----------
local backtrack = ui.new_checkbox("lua", "b", "Extented backtrack")
...
-----------回溯拓展-----------
-----------腿部拓展-----------
local aa_legs = ui.new_multiselect("lua", "b", "Extented leg movement", {"Static legs in air", "Break leg movement"})
...
fakelag = ui.reference("AA", "Fake lag", "Limit")
 client.set_event_callback("net_update_end", function()
    if state_anims and contains(ui.get(aa_legs), "Break leg movement") then
...
client.set_event_callback("pre_render", function()
    if contains(ui.get(aa_legs), "Static legs in air") then
 ...
 local function legfucker()
  if contains(ui.get(aa_legs), "Break leg movement") then
...
local hotkeySlowWalk = ui.new_hotkey("LUA", "B", "Cripple Slow walk")
local sliderSlowWalk = ui.new_slider("LUA", "B", "Cripple Slow walk Limit", 1, 110, 40, true, "u")
...
local on_run_command = function(cmd)
    ...
    if not ui.get(hotkeySlowWalk) then
        setSpeed(450)
    else
        setSpeed(ui.get(sliderSlowWalk))
    end
end
...
ui_new_label("LUA", "B", "ZHAG-YAW color")
local ZHAG_yaw_clr = ui_new_color_picker("LUA", "B", "ZHAG yaw color", 255, 166, 180, 255)
ui_new_button("LUA", "B", "Import recommended settings", function()
    ...
    ui_set(side_arrows, true)
end)
-----------高级Peek-----------
local ref_dt, ref_dt_key = ui.reference("RAGE", "Other", "Double tap")  
local exploit = ui.new_checkbox("LUA", "B", "ideal Peek ( 1 tick )")
local exploit_key = ui.new_hotkey("LUA", "B", " ", true)
...
local function setup()
    ...
    ui.set(ref_dt_key, getstate and 'Always on' or 'toggle')
end
...
local function dt_indicator()
    ...
    renderer.text(wnd.x + wnd.w / 2, wnd.y + 9, 255, 255, 255, 255, "c", 0, string.format("ZHAG DT (%s%%) | ticks: %s", ui_get(ref_dt_hc), ui_get(ref_maxprocessticks)))
...
local function manual_aa_paint()
    ...
    if m_state == 1 then renderer.text(w/2 - 40, h - 542.5, 0, 0, 0, 125, "+c", 0, "⯇") end
    if m_state == 2 then renderer.text(w/2 + 40, h - 542.5, 0, 0, 0, 125, "+c", 0, "⯈") end

    if m_state == 1 then renderer.text(w/2 - 40, h - 542.5, r, g, b, alpha, "+c", 0, "⯇") end
    if m_state == 2 then renderer.text(w/2 + 40, h - 542.5, r, g, b, alpha, "+c", 0, "⯈") end
end
...
local function set_anti_aim(type)
    ...
    elseif type == "left" then
        ...
        ui_set(ref_yawjadd, math.random(-5,7))
        ui_set(ref_bodyyaw, "jitter")
        ui_set(ref_bodyyawadd, 119)
        ui_set(ref_fs_bodyyaw, false)
        --ui_set(ref_lby_target, "Eye yaw")
    ...
    elseif type == "right" then
        ...
        ui_set(ref_yawadd, 6)
        ui_set(ref_yawj, "Offset")
        ui_set(ref_yawjadd, 7)
        ui_set(ref_bodyyaw, "jitter")
		ui_set(ref_bodyyawadd, -119)
        ui_set(ref_fakelimit, math.random(22,27))
...
local function draw()
    ...
    local m_r, m_g, m_b, m_a = ui_get(ZHAG_yaw_clr)
    ...
    renderer.text(w / 2, h / 2 + 40, m_r, m_g, m_b, 255, "c", 0, "ZHAG-YAW")
    ...
    if ui_get(side_arrows) then
        if not ui_get(manual_aa) or ui_get(manual_state) == 0 then
            if brute.best_angle ~= 0 or brute.indexed_angle ~= 0 then
                ...
                if ui_get(ref_bodyyawadd) == 119 then
                    renderer.text(w / 2 + 40, h - 542.5, 255, 255, 255, 255, "cb+", 0, ">")
                    renderer.text(w / 2 - 40, h - 542.5, s_r, s_g, s_b, s_a, "cb+", 0, "<")
                else
                    renderer.text(w / 2 + 40, h - 542.5, s_r, s_g, s_b, s_a, "cb+", 0, ">")
                    renderer.text(w / 2 - 40, h - 542.5, 255, 255, 255, 255, "cb+", 0, "<")
                end
    ...
    if includes(ui_get(indicators), "Doubletap") and ui_get(ref_dt_key) and ui_get(ref_dt) then
        if doubletap_charged() then
            renderer.text(w / 2, h / 2 + 55 + (h_index * 12), 255, 255, 255, 255, "c", 0, string.format("rapid [%s]", ui_get(ref_maxprocessticks)))
        else
...
local function fakelag()
    ...
    if trace_positions(player_pos.x, player_pos.y, player_pos.z, player_pos_2.x, player_pos_2.y, player_pos_2.z, player_pos_3.x, player_pos_3.y, player_pos_3.z, extrap_pos.x, extrap_pos.y, extrap_pos.z) then
        if end_choke_cycle ~= 2 then
            ...
            ui_set(ref_fl_amt, "Fluctuate")
            ui_set(ref_fl_limit, 15)
            ui_set(ref_fl_var, 25)
        end
    else
        end_choke_cycle = 0
